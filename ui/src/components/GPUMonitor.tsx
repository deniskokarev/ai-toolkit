...
const GpuMonitor: React.FC = () => {
  ...
  const content = useMemo(() => {
    ...
    if (!gpuData.hasNvidiaSmi && !gpuData.hasRocmSmi && !gpuData.hasAmdSmi && !gpuData.isMac) {
      return (
        <div className="bg-yellow-900 border border-yellow-700 text-yellow-300 px-4 py-3 rounded relative" role="alert">
    ...
    if (gpuData.gpus.length === 0) {
      const tool = gpuData.hasNvidiaSmi ? 'nvidia-smi' : gpuData.hasRocmSmi ? 'rocm-smi' : gpuData.hasAmdSmi ? 'amd-smi' : 'GPU tool';
      return (
        <div className="bg-yellow-900 border border-yellow-700 text-yellow-300 px-4 py-3 rounded relative" role="alert">
          <span className="block sm:inline">No GPUs found, but {tool} is available.</span>
        </div>
      );
    }
